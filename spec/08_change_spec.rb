...
describe 'lucky numbers with rotate! method' do
  ...
  context 'when rotating the array of lucky numbers' do
    ...
    it 'will change the first value to 7' do
      expect { lucky_numbers.rotate! }.to change { lucky_numbers.first }.to(lucky_numbers[1])
    end

    # remove the 'x' before running this test
    it 'will change the last value to 3' do
      expect { lucky_numbers.rotate! }.to change { lucky_numbers.last }.to(lucky_numbers[0])
    end

    # remove the 'x' before running this test
    it 'will change the first value to 7 and last value to 3' do
      expect { lucky_numbers.rotate! }.to change { lucky_numbers.first }.to(lucky_numbers[1]).and change { lucky_numbers.last }.to(lucky_numbers[0])
    end
  end
end
