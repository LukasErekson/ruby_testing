...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:guessing_game) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(guessing_game.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:guessing_game) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(guessing_game.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:guessing_game) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(guessing_game.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:guessing_game) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(guessing_game.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:guessing_game) { described_class.new(0, 9, random_number, 7) }
      ...
      it 'is game over' do
        allow(random_number).to receive(:value).and_return(guessing_game.guess)
        expect(guessing_game).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        ...
        expect(guessing_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect { low_guess_game.update_range }.to change(low_guess_game, :min).from(0).to(5)
      end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'updates max to 8' do
        expect { high_guess_game.update_range }.to change(high_guess_game, :max).from(9).to(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guess_one_below) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        expect { guess_one_below.update_range }.to change(guess_one_below, :min).from(5).to(8)
      end

      it 'does not update max' do
        expect { guess_one_below.update_range }.not_to change(guess_one_below, :max)
      end
    end

    context 'when the guess is 8, min=7 and max=9' do
      subject(:guess_one_above) { described_class.new(8, 9, number_range, 8) }

      it 'does not update min' do
        expect { guess_one_above.update_range }.not_to change(guess_one_above, :min)
      end

      it 'updates max to the same value as min' do
        expect { guess_one_above.update_range }.to change(guess_one_above, :max).from(9).to(8)
      end
    end
  end
end
