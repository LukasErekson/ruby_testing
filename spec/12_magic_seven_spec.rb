...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    let(:my_game) { described_class.new(7) }
    context 'when the random number is 7' do
      it 'stores 7 as an attribute' do
        expect(my_game.random_number).to be 7
      end
    end
  end
  ...
  describe '#play' do
    context 'when the random number is 4' do
      ...
      it 'will return 7' do
        expect(described_class.new(4).play).to eq 7
      end
    end

    context 'when the random number is 3167' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(described_class.new(3167).play).to eq 7
      end
    end

    context 'when the random number is 8675309' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(described_class.new(8675309).play).to eq 7
      end
    end
  end
end
