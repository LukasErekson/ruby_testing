...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      let(:wrong_guess) { described_class.new(5, '6') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        second_input = 'b'
        valid_input = '2'
        allow(game_loop).to receive(:player_input).and_return(first_input, second_input, valid_input)
      end

      it 'completes loop and displays error message twice' do
        allow(game_loop).to receive(:puts).with('Input error!').twice
        game_loop.player_turn
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      let(:game_four) { described_class.new(5, '5', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        output_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_four.final_message }.to output(output_phrase).to_stdout
      end
    end
  end
end
